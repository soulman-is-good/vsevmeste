...
X3::import('@app:extensions:image:Image_Driver.php');
...
class Image {
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ($check === FALSE)
			throw new X3_Exception('image getimagesize missing');

		// Check to make sure the image exists
		if ( ! is_file($image))
			throw new X3_Exception('image file not found');
		...
		if ( ! is_array($image_info) OR count($image_info) < 3)
			throw new X3_Exception('image file unreadable');

		// Check to make sure the image type is allowed
		if ( ! isset(Image::$allowed_types[$image_info[2]]))
			throw new X3_Exception('image type not allowed');
        ...
        // Load the driver
        X3::import("@app:extensions:image:drivers:$driver.php");
		...
		if ( ! ($this->driver instanceof Image_Driver))
			throw new X3_Exception('image driver must be implement Image_Driver class');
	}
	...
	public function __get($property)
	{
		...
		{
			throw new X3_Exception('invalid property');
		}
	}
	...
	public function resize($width, $height, $master = NULL)
	{
		if ( ! $this->valid_size('width', $width))
			throw new X3_Exception('image invalid width');

		if ( ! $this->valid_size('height', $height))
			throw new X3_Exception('image invalid height');

		if (empty($width) AND empty($height))
			throw new X3_Exception('image invalid dimensions');
		...
		elseif ( ! $this->valid_size('master', $master))
			throw new X3_Exception('image invalid master');
	...
	{
		if ( ! $this->valid_size('width', $width))
			throw new X3_Exception('image invalid width', $width);

		if ( ! $this->valid_size('height', $height))
			throw new X3_Exception('image invalid height', $height);

		if ( ! $this->valid_size('top', $top))
			throw new X3_Exception('image invalid top', $top);

		if ( ! $this->valid_size('left', $left))
			throw new X3_Exception('image invalid left', $left);
	...
	public function flip($direction)
	{
		if ($direction !== self::HORIZONTAL AND $direction !== self::VERTICAL)
			throw new X3_Exception('image invalid flip');
	 ...
	 *
	 * @throws  X3_Exception
	 * @param   integer  procent 0-100
	 * @return  object
	 */
	public function gray($percent=100)
	{
                if($percent<0 || $percent>100)
                    throw new X3_Exception('Image grayscale wrong percent value. Must be between 0 and 100');
                    $this->actions['gray'] = $percent;
	...
	public function save($new_image = FALSE, $chmod = 0644, $keep_actions = FALSE)
	{
		...
		if ( ! is_writable($dir))
			throw new X3_Exception('image directory unwritable');
