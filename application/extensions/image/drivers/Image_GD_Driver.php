...
class Image_GD_Driver extends Image_Driver {
    ...
    public function __construct() {
        ...
        if (!function_exists('gd_info'))
            throw new X3_Exception('image gd requires v2');
    ...
    public function gray($percent) {
        ...
        for ($i = 0; $i < $imgw; $i++) {
            for ($j = 0; $j < $imgh; $j++) {
                ...
                $rgb = ImageColorAt($this->tmp_image, $i, $j);
                ...
                $val = imagecolorallocatealpha($this->tmp_image, $g, $g, $g,$aa);

                // set the gray value

                imagesetpixel($this->tmp_image, $i, $j, $val);
            }
        }
        $img = $this->imagecreatetransparent($imgw, $imgh);
        if ($status = imagecopy($img, $this->tmp_image, 0, 0, 0, 0,$imgw,$imgh)) {
            // Swap the new image for the old one
            imagedestroy($this->tmp_image);
            $this->tmp_image = $img;
        }        
    ...
    public function sharpen($amount) {
        ...
        if (!function_exists('imageconvolution'))
            throw new X3_Exception('image unsupported method');
