...
function process_request($order_id,$currency_code,$amount,$config_file,$b64=true) {
	...
	$config['PRIVATE_KEY_FN'] = DOCROOT.$config['PRIVATE_KEY_FN'];
	$config['XML_TEMPLATE_FN'] = DOCROOT.$config['XML_TEMPLATE_FN'];
	$config['XML_COMMAND_TEMPLATE_FN'] = DOCROOT.$config['XML_COMMAND_TEMPLATE_FN'];
	$config['PUBLIC_KEY_FN'] = DOCROOT.$config['PUBLIC_KEY_FN'];
	$kkb = new KKBSign();
	$kkb->invert();
...
function process_check($reference, $approval_code, $order_id, $currency_code, $amount, $config_file) {
//echo $reference.' '.$approval_code.' '.$order_id.' '.$currency_code.' '.$amount.' ';exit;
	if(!$reference) return "Empty Transaction ID";
