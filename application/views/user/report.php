...
<div class="eksk-wnd">
    <div class="head">
        <div class="buttons">
            ...
            <div class="wrapper inline-block"><a class="button inline-block" id="create_forum" href="/report/create.html"><?=X3::translate('Создать')?></a></div>
            <?endif;?>
        </div>
        <h1><?=X3::translate('Отчеты');?><?=" ".$from?></h1>
    </div>
    <div class="content">
            <?while($model = mysql_fetch_object($models)):
                //$user = (object)X3::db()->fetc2h("SELECT id, CONCAT(name,' ',surname) name, image, role FROM data_user WHERE id = ".$model->user_id);
                ...
                ?>
            <div class="message_block<?=X3::db()->count("SELECT id FROM user_report_stat WHERE user_id=$id AND report_id=$model->id")>0?'':' unread'?>"style="min-height:0">
            <table class="admin-list">
                <tr>
                    ...
                    <td class="text" <?=$i>0?'style="border-top:none"':''?>>
                            <p><a target="_blank" href="/report:<?=$model->id?>.html" onclick="$(this).parents('.message_block').removeClass('unread');setTimeout(function(){message_count()},1000)"><?=nl2br($model->title);?></a></p>
                    ...
                    <td class="ops" <?=$i>0?'style="border-top:none"':''?>>
                        <?if(X3::user()->id == $model->user_id || X3::user()->isAdmin()):?>
                            <?if($model->status=='0'):?>
                                <a href="/report/create/id/<?=$model->id?>.html"><span><?=X3::translate('Редактировать')?></span></a>
                                <a href="/report/send/id/<?=$model->id?>.html"><span><?=X3::translate('Опубликовать')?></span></a>
                                <a href="/report/delete/id/<?=$model->id?>.html"><span><?=X3::translate('Удалить')?></span></a>
                            <?else:?>
                                <em style="display:block;margin-bottom:15px"><?=X3::translate('Опубликовано')?></em>
                                <?if(X3::user()->isAdmin() || X3::user()->id == $model->user_id):?>
                                <a href="/report/delete/id/<?=$model->id?>.html"><span><?=X3::translate('Удалить')?></span></a>
                                <?endif;?>
                            <?endif;?>
                        <?endif;?>
